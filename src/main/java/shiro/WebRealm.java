...
public class WebRealm extends AuthorizingRealm {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(
		...
		if (username.length() > 0) {
			...
			authenticationInfo.setStringPermissions(new HashSet<String>());
			//
	...
	protected AuthenticationInfo doGetAuthenticationInfo(
		...
		authenticationInfo.setCredentialsSalt(ByteSource.Util.bytes(svc
				.getSalt()));
		return authenticationInfo;
	}
	...
	private boolean hasRoleWithNotOperator(PrincipalCollection principals,
			String roleIdentifier) {
		if (roleIdentifier.startsWith(NOT_OPERATOR)) {
			return !super.hasRole(principals,
					roleIdentifier.substring(NOT_OPERATOR.length()));
		} else {
